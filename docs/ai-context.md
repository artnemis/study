# Project: Artnemis

## Vision

Artnemis is an open-source AI-powered platform to help users prepare for exams.

Repository policy:

* public on GitHub
* MIT licensed

It is NOT a generic learning platform.
It is an "AI exam preparation engine" with collaborative modules.

---

## Core Concepts

### Study Module

A Study Module is the main unit of the system.

It contains:

* study content
* AI configuration
* quiz generation rules
* collaboration settings
* curriculum (structured sections → lessons with types, duration, progress)
* study materials (uploaded files with AI-extracted topics and cost estimation)

---

### Module Visibility

* public → visible and forkable by anyone
* private → accessible only by members

---

### Roles

* owner → full control
* editor → can modify content and config
* viewer → read-only access

---

### Study Plan

A structured schedule generated by AI based on:

* exam date
* topics
* user availability
* template (multiple-choice / free-response / mixed)

---

### Quiz System

A quiz contains:

* template: multiple-choice, free-response, or mixed
* multiple questions
* each question:

  * type: multiple-choice or free-response
  * 4 options (multiple-choice) or 0 options (free-response)
  * 1 correct answer
  * explanation

---

### Adaptive Learning

The system must:

* track user mistakes
* prioritize weak topics
* adjust difficulty dynamically

---

### AI Provider (Pluggable)

Interface:

```ts
interface AIProvider {
  generateText(prompt: string): Promise<string>
  generateQuiz(input: QuizInput): Promise<Quiz>
  generatePlan(input: PlanInput): Promise<StudyPlan>
}
```

---

### AI Modes

* shared API key (default, limited)
* user API key (advanced mode)

---

## Collaboration

Users can:

* create modules
* invite collaborators
* fork public modules

---

## Tech Stack

Frontend:

* Next.js (App Router)
* TypeScript
* proxy-based launch gate via `COMING_SOON_ENABLED` (proxy.ts convention)

Backend:

* Next.js API routes

Database:

* PostgreSQL
* local JSON fallback for MVP/demo mode when DATABASE_URL is missing

Auth:

* NextAuth.js v5 (Auth.js) with Credentials provider
* JWT session strategy
* Sign-in / Sign-up pages
* Protected routes via session checks

i18n:

* Client-side React Context provider
* 5 languages: IT, EN, FR, ES, DE
* Dictionary-based translation system
* Locale stored in cookie + user settings

---

## Design Principles

* modular architecture
* pluggable AI providers
* strict typing
* simple and clear logic

---

## Project Structure

* /core/ai → AIProvider contracts and provider adapters
* /core/auth → user and settings types
* /core/module → module creation, lookup, permissions, invites
* /core/quiz → quiz types, generation, validation (supports templates: MC/FR/mixed)
* /core/plan → study plan types and generation (supports templates)
* /hooks → client hooks only, no business logic
* /lib/i18n → dictionaries, locale context, types
* /app/api → route handlers and backend-for-frontend layer
* /app/api/auth → NextAuth routes and sign-up endpoint
* /app/api/profile → user profile and API key management
* /app/api/modules/[moduleId]/materials → file upload endpoint
* /app/api/modules/[moduleId]/topics → AI topic extraction endpoint
* /app/auth → sign-in and sign-up UI pages
* /app/profile → user profile page
* /app/modules/[moduleId]/curriculum → structured course view (Udemy-style sections)
* /app/modules/[moduleId]/materials → study material upload and topic extraction
* /app/legal → privacy policy, terms of service, cookie policy
* /app/coming-soon → launch gate page shown when the site is temporarily closed
* /proxy.ts → global request gate for launch / maintenance mode (replaces middleware.ts)
* /docs → public repo documentation and onboarding

---

## MVP Scope

* create module
* generate plan
* generate quizzes
* basic collaboration (invite)
* list visible modules
* inspect module members
* PostgreSQL-backed server runtime with local fallback
* multi-page UI with clean navigation

### Site Structure

The MVP ships the following pages:

| Route               | Purpose                                              |
|---------------------|------------------------------------------------------|
| `/`                 | Public landing page – hero, feature highlights, CTA  |
| `/auth/sign-in`     | Sign in with email and password                      |
| `/auth/sign-up`     | Register new account                                 |
| `/coming-soon`      | Public launch gate page shown when access is paused  |
| `/dashboard`        | Authenticated overview – recent modules, quick stats |
| `/modules`          | Browse, search and create study modules              |
| `/modules/[id]`     | Module detail – content, members, invite, quiz, plan, curriculum, materials |
| `/modules/[id]/curriculum` | Structured course view with sections, lessons, progress tracking |
| `/modules/[id]/materials`  | Upload study materials, AI topic extraction with cost estimation |
| `/quiz`             | Standalone quiz generator (with template selector) |
| `/plans`            | Standalone study plan generator (with template selector) |
| `/profile`          | User profile – language, OpenAI API key management   |
| `/legal/privacy`    | Privacy policy page                                  |
| `/legal/terms`      | Terms of service page                                |
| `/legal/cookies`    | Cookie policy page                                   |

Shared chrome: persistent top navbar + footer with legal links and cookie consent banner.
Each section is self-contained; users are guided to features via clear CTAs.

---

## Non Goals

* no social feed
* no marketplace
* no complex gamification (initially)

---

## Coding Rules

* no "any"
* small pure functions
* validate all AI outputs
* no business logic in UI

---

## Testing Strategy

We require high test coverage (>90%).

### Rules

* All core logic must be unit tested
* Validation must be fully tested
* Edge cases must be covered
* No untested business logic

### Test Types

* unit tests → core logic
* integration tests → API routes
* no heavy UI testing

### Mocking

* AIProvider must always be mocked
* no real API calls in tests

### Implemented Core Contracts

* getModuleById must include members
* private modules require membership for read access
* invites create memberships through validated tokens
* quiz and study plan AI output must be validated before use
* modules list returns public modules plus requester-visible private modules
* route handlers are public endpoints and must sanitize failures

### Coverage Target

* global: 90%+
* core modules: 100%

### Important

Tests must verify behavior, not implementation.


---

## Important

AI output must never be trusted blindly.
Always validate and sanitize responses.

Route handlers must avoid leaking infrastructure details in error responses.
